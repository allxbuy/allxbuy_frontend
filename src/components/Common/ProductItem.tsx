...
const ProductItem = ({ item }: { item: Product }) => {
  ...
  // update product details for the individual product page
  ...
  return (
    <div className="group flex flex-col h-full">
      ...
      <div className="relative overflow-hidden flex items-center justify-center rounded-2xl bg-slate-50 min-h-[270px] mb-5 transition-colors duration-300 group-hover:bg-slate-100">
        <Image 
          src={item.imgs?.previews[0] || "/images/placeholder.png"} // Fallback added just in case
          alt={item.title} 
          width={250} 
          height={250} 
          className="object-contain transition-transform duration-500 ease-out group-hover:scale-105 mix-blend-multiply"
        />

        {/* Hover Action Buttons */}
        <div className="absolute left-0 bottom-0 translate-y-full w-full flex items-center justify-center gap-2.5 pb-6 ease-out duration-300 opacity-0 group-hover:translate-y-0 group-hover:opacity-100">
          ...
          <button
            ...
            className="flex items-center justify-center w-10 h-10 rounded-full shadow-md text-slate-600 bg-white transition-all duration-200 hover:bg-blue-600 hover:text-white hover:-translate-y-1"
          >
          ...
          <button
            ...
            className="inline-flex font-semibold text-sm py-2 px-6 rounded-full shadow-md bg-blue-600 text-white transition-all duration-200 hover:bg-blue-700 hover:-translate-y-1"
          >
            Add to cart
          </button>
          ...
          <button
            ...
            className="flex items-center justify-center w-10 h-10 rounded-full shadow-md text-slate-600 bg-white transition-all duration-200 hover:text-red-500 hover:bg-red-50 hover:-translate-y-1"
          >
            <svg className="fill-current w-4 h-4" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <div className="flex items-center gap-2 mb-3">
        ...
        <p className="text-sm text-slate-500">({item.reviews || 0})</p>
      </div>

      {/* Product Title (Line-clamped for grid consistency) */}
      <h3 className="font-semibold text-base text-slate-900 leading-snug mb-2 flex-grow transition-colors duration-200 hover:text-blue-600 line-clamp-2">
        <Link 
          href="/shop-details" 
      ...
      <div className="flex items-center gap-2.5 mt-auto">
        ...
        {item.price && (
          <span className="font-medium text-sm text-slate-400 line-through">${item.price}</span>
        )}
      </div>
    </div>
  );
